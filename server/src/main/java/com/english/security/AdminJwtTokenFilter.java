...
public class AdminJwtTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request,
        ...
        try {
            ...
            if (!jwtUtil.validateRegisterAdminToken(jwt)) {
            ...
            if (isUserRegistered && path.startsWith("/api/admin/register")) {
                throw new AuthenticationException("The user is already registered");
            }

            if (login == null || !isUserRegistered && !path.startsWith("/api/admin/register")) {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        String path = request.getRequestURI();
        return !path.startsWith("/api/admin/register");
    }
}
